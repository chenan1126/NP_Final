...
def send_order():
    ...
    try:
        ...
        client.close()
        messagebox.showinfo("訂單", f"訂單已送出")
    except socket.error as e:
        messagebox.showerror("錯誤", f"訂單發送失敗: {e}")

def delete_order():
    order_id = delete_order_var.get()
    if not order_id:
        messagebox.showerror("錯誤", "請輸入訂餐人姓名")
        return
    delete_command = f"DELETE_ORDER: {order_id}"
    try:
        client = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        client.connect(('localhost', 12345))
        client.send(delete_command.encode())
        response = client.recv(1024).decode()
        client.close()
        if response == "SUCCESS":
            messagebox.showinfo("刪除訂單", "訂單已刪除")
        else:
            messagebox.showerror("錯誤",response)
    except socket.error as e:
        messagebox.showerror("錯誤", f"刪除訂單失敗: {e}")
...
# 刪除訂單框架
delete_order_frame = tk.Frame(root, bg='#ededad')
delete_order_frame.grid(row=6, column=0, columnspan=2, pady=5, padx=10, sticky='n')
tk.Label(delete_order_frame, text="訂單人姓名：", bg='#ededad', fg='#333', font=('微軟正黑體', 12)).pack(side=tk.LEFT)
delete_order_var = tk.StringVar()
tk.Entry(delete_order_frame, textvariable=delete_order_var, font=('微軟正黑體', 12), width=30).pack(side=tk.LEFT, padx=10)
tk.Button(delete_order_frame, text="刪除訂單", font=('微軟正黑體', 12), command=delete_order).pack(side=tk.LEFT, padx=10)
